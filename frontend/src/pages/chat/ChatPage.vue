...
const connectWebSocket = () => {
  ...
  try {
    ...
    ws.onopen = () => {
      console.log("Connected to LangGraph AI Chatbot WebSocket")
    }
    ...
    ws.onerror = () => {
      console.warn("AI Chatbot WebSocket error — fallback active")
    }
...
async function sendMessage(content?: string) {
 ...
 // Smart assistant fallback with personalized context awareness
 isTyping.value = true
 setTimeout(async () => {
 isTyping.value = false
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <div class="flex items-center gap-1.5">
          ...
          <span class="text-[0.72rem] text-ink-muted font-data">LangGraph Multi-Agent · ICMR-NIN Grounded</span>
        </div>
      </div>
    </div>
